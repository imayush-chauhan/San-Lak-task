...
class HomeView extends StatelessWidget {
        ...
        child: GridView.builder(
          ...
          itemBuilder: (context, index) => Padding(
            ...
            child: InkWell(
              onTap: (){
                print("hehe");
                _showModalBottomSheetSingle(context, viewModel, index);
              },
              child: ItemCard(
                context: context,
  ...
  // Basket Menu
  void _showModalBottomSheetSingle(BuildContext context, HomeViewModel viewModel,int index) {
    showModalBottomSheet(
      context: context,
      shape: RoundedRectangleBorder(borderRadius: context.normalBorderRadius),
      backgroundColor: context.groupedBackground,
      builder: (context) =>
          StatefulBuilder(builder: (context, StateSetter setState) {
        return _basketItemSingle(context, viewModel, index, setState);
      }),
    );
  }
  ...
  FlipInY _emptyBasket(BuildContext context) => FlipInY(
   ...
   _basketItems(BuildContext context, HomeViewModel viewModel,
  ...
  JelloIn _basketItemSingle(BuildContext context, HomeViewModel viewModel, int index,
          StateSetter setState) =>
      JelloIn(
        child: Card(
          elevation: 5,
          ...
          child: Padding(
            ...
            child: ListTile(
              leading: ResponsiveImage(
                  aspectRaito: 1,
                  imageUrl: viewModel.items[index].image ?? ""),
              title: ProductName(
                data: viewModel.items[index].title ?? 'Title is null !',
              ),
              subtitle:
              _priceCountAndButtonsSingle(context, viewModel, index, setState),
            ),
          ),
        ),
      );
  ...
  Column _priceCountAndButtonsSingle(BuildContext context, HomeViewModel viewModel,
          int index, StateSetter setState) =>
      Column(
        children: [
          _pricesAndCountSingle(context, viewModel, index),
          context.emptySizedHeightBoxLow,
          _buttonsSingle(setState, viewModel, index, context),
        ],
      );
      ...
      Wrap(
        children: [
          ...
          PrimaryBoldText(
            context: context,
            data:
                'Price: ${viewModel.items[index].productPrice.toString()} ₹',
          ),
          context.emptySizedWidthBoxLow2x,
          PrimaryBoldText(
            context: context,
            data:
                'Count: ${viewModel.items[index].count.toString().substring(0, 1)}',
          ),
        ],
      );

  Row _buttonsSingle(StateSetter setState, HomeViewModel viewModel, int index,
          BuildContext context) =>
      Row(
        children: [
          GestureDetector(
            onTap: () => setState(
                () => viewModel.incrementCount(viewModel.items[index])),
            child: SpecialIconCard(context: context, icon: Icons.add),
          ),
          // Count card
          SpecialTextCard(
            context: context,
            data: viewModel.items[index].count.toString().substring(0, 1),
          ),
          // Deincrement count button
          GestureDetector(
            onTap: () {
              setState(() {
                viewModel.deIncrementCount(viewModel.items[index]);
                if (viewModel.items[index].count == 0) {
